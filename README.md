...
```
> Terraform сможет прочитать содержимое этих переменных и применить их для авторизации на YC в том случае, если экспорт переменных выполнен в той же сессии терминала, в которой запускается Terraform. Таким образом, указывать в `main.tf` ссылки на параметры авторизации становится не обязательным.

2. В данной конфигурации применены группы безопасности, описание которых дано в разделе 5 этого ридми-файла.
3. Для инстансов Frontend и Backend использован кастомный образ на основе ОС Centos7. User - centos, ключ SHH использовать ваш.
4. Для иснтанса Natgw использован образ id=fd80mrhj8fl2oe87o4e1. User = ubuntu, ключ SHH использовать ваш.
